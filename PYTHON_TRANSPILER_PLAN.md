...
1.  **Remove JS-specific and Browser-specific standard library APIs:**
    ...
    *   **Note on Standard Library Generation:**
        *   The files in `libraries/stdlib/python/src/generated/` (like `_ArraysJs.kt`) are automatically generated by `GenerateStandardLib.kt`. They should not be manually edited.
        *   We must update `libraries/tools/kotlin-stdlib-gen/src/templates/dsl/CommonTypes.kt` to include `Platform.Python` and `KotlinTarget.Python`.
        *   We must audit the templates in `libraries/tools/kotlin-stdlib-gen/src/templates/` (e.g., `Arrays.kt`, `Comparables.kt`) to ensure that any code currently gated specifically for JS (like `JsMath.min`) is either gated correctly or mapped to an expect/actual or inline python intrinsic (e.g. `min(a, b)`).
        *   We must re-run `:kotlin-stdlib-gen:run` to regenerate the `python` targets cleanly.

    *   **Note on @PythonIntrinsic vs TODO():**
        *   Currently, the codebase contains remnants like `TODO("[]")` from when it was cloned from Kotlin/JS. The JS backend has a `JsIntrinsic` annotation which allows the compiler to intercept specific functions and generate intrinsic IR calls instead of standard function calls.
        *   We must implement an `@PythonIntrinsic` annotation (similar to `@JsIntrinsic`).
        *   We must create a `PythonIntrinsic` lowering phase in `compiler/ir/backend.python` that intercepts calls to functions annotated with `@PythonIntrinsic` and replaces them with native Python constructs (like list creation `[]`, dict creation `{}`, or `type(None)` mappings).
        *   The `krzema12/kotlin-python` repository handles this similarly: its compiler backend maps specific Kotlin operations (like `plus`, `println`, array instantiation) directly into Python AST nodes within the IR Transformer (e.g. mapping `println` to `PythonCall(PythonName("print"))`). We will expand this concept using the `@PythonIntrinsic` annotation to allow standard library functions to define Python code directly or signal the compiler to emit specific AST nodes.
