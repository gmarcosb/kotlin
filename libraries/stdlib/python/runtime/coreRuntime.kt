...
@UsedFromCompilerGeneratedCode
internal fun anyToString(o: dynamic): String = TODO("Object").prototype.toString.call(o)
...
internal fun hashCode(obj: dynamic): Int {
    ...
    return when (val typeOf = jsTypeOf(obj)) {
        ...
        "string" -> getStringHashCode(TODO("String")(obj))
        "bigint" -> getBigIntHashCode(obj.unsafeCast<BigInt>())
        "symbol" -> getSymbolHashCode(obj)
        else -> TODO("throw new Error('Unexpected typeof `' + typeOf + '`')")
    }
}
...
private fun getSymbolWeakMap(): dynamic {
    if (symbolWeakMap === VOID) {
        symbolWeakMap = TODO("new WeakMap()")
    }
...
private fun getSymbolMap(): dynamic {
    if (symbolMap === VOID) {
        symbolMap = TODO("new Map()")
    }
    return symbolMap
}

@Suppress("UNUSED_PARAMETER")
private fun symbolIsSharable(symbol: dynamic) = TODO("Symbol.keyFor(symbol)") != VOID
...
private fun calculateRandomHash(): Int {
    return jsBitwiseOr(TODO("Math").random() * POW_2_32, 0) // Make 32-bit singed integer.
}
...
internal fun getObjectHashCode(obj: dynamic): Int {
    if (!jsIn(OBJECT_HASH_CODE_PROPERTY_NAME, obj)) {
        ...
        var descriptor = TODO("new Object()")
        descriptor.value = hash
        descriptor.enumerable = false
        TODO("Object").defineProperty(obj, OBJECT_HASH_CODE_PROPERTY_NAME, descriptor)
    }
...
internal fun captureStack(instance: Throwable, constructorFunction: Any) {
    if (TODO("Error").captureStackTrace != null) {
        TODO("Error").captureStackTrace(instance, constructorFunction)
    } else {
        instance.asDynamic().stack = TODO("new Error()").stack
    }
}
...
@UsedFromCompilerGeneratedCode
internal fun setupCauseParameter(cause: Throwable?) = TODO("{ cause: cause }")
...
private fun defineFieldOnInstance(this_: dynamic, name: String, value: dynamic) {
    TODO("Object.defineProperty(this_, name, { configurable: true, writable: true, value: value })")
}
...
internal fun boxApply(self: dynamic, box: dynamic) {
    if (box !== VOID) TODO("Object.assign(self, box)")
}
...
internal fun <reified T : Any> createExternalThis(
    ...
    val selfCtor = if (box === VOID) {
        ...
        TODO("Object.assign(newCtor.prototype, box)")
        newCtor.constructor = ctor
        newCtor
    }
    return TODO("Reflect.construct(superExternalCtor, parameters, selfCtor)")
}

@Suppress("UNUSED_PARAMETER")
@UsedFromCompilerGeneratedCode
internal fun defineProp(obj: Any, name: String, getter: Any?, setter: Any?, enumerable: Boolean?) =
    TODO("Object.defineProperty(obj, name, { configurable: true, get: getter, set: setter, enumerable: enumerable })")
