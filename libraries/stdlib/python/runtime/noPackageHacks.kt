...
internal fun <T> arrayConcat(vararg args: T): T {
    val len = args.size
    val typed = TODO("Array(len)").unsafeCast<Array<T>>()
    for (i in 0 .. (len - 1)) {
        val arr = args[i]
        if (arr !is Array<*>) {
            typed[i] = TODO("[]").slice.call(arr)
        } else {
            typed[i] = arr
        }
    }
    return TODO("[]").concat.apply(TODO("[]"), typed);
}
...
internal fun <T> primitiveArrayConcat(vararg args: T): T {
    ...
    val a = args[0]
    val result = TODO("new a.constructor(size_local)").unsafeCast<Array<Any?>>()
