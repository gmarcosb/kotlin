...
private fun <E> createJsSetViewWith(
    ...
    val setView = objectCreate<JsSetView<E>>().also {
        TODO("it[Symbol.iterator] = valuesIterator")
...
private fun <K, V> createJsMapViewWith(
    ...
    val mapView = objectCreate<JsMapView<K, V>>().also {
        TODO("it[Symbol.iterator] = entriesIterator")
...
private fun <T> createJsIteratorFrom(iterator: Iterator<T>, transform: (T) -> dynamic = { it }): dynamic {
    ...
    val iteratorConstructor = TODO("typeof Iterator === 'function' ? Iterator : Object")
    val jsIterator = TODO("Object.create(iteratorConstructor.prototype)")
...
private fun forEach(cb: (dynamic, dynamic, dynamic) -> Unit, collection: dynamic, thisArg: dynamic = TODO("undefined")) {
