...
internal actual object MonotonicTimeSource : DefaultTimeSource, TimeSource.WithComparableMarks {  // TODO: interface should not be required here
    ...
    private val actualSource: DefaultTimeSource = run {
        val isNode: Boolean = TODO("typeof process !== 'undefined' && process.versions && !!process.versions.node")

        if (isNode)
            HrTimeSource(TODO("process").unsafeCast<Process>())
        else
            TODO("typeof self !== 'undefined' ? self : globalThis")
